 ...
 * --------------------------------------------------------------------------------------------------
 *
 * Custom Modification Author: [R. Irujo]
 * Inception:                  05.09.2012
 * 
 * Purpose:                    All timeout and Socket related errors related to the NRPE Agent
 *                             will no longer return back in Nagios/Icinga as CRITICAL errors,
 *                             instead they will be returned back as UNKNOWN errors. This has
 *                             been done to significantly cut down on the number of alert
 *                             storms that can occur due to network or host related issues.
 *                             Since most Monitoring Teams are the only groups that are  
 *                             required to recieve UNKNOWN error messages,
 *                             this will cut down significantly on the amount of false alerts
 *                             that other groups will recieve due to network outages.
 *
 *
 * Last Modified:              05.09.2012 - [R. Irujo]
 *                             v1.0
 *                             Changed all STATE_CRITICAL return values to STATE_UNKNOWN.
 *
 *                             06.11.2012 - [R. Irujo]
 *                             v1.1
 *                             'check_nrpe' plugin recompiled again to apply the changes made
 *                             in the 'utils.c' file.
 *
 ***************************************************************************************************/
...
/* Timeout_return_code changed to STATE_UNKNOWN - [R. Irujo] - 05.09.2012 */
/* int timeout_return_code=STATE_CRITICAL; */
int timeout_return_code=STATE_UNKNOWN;
int sd;
...
int main(int argc, char **argv){
	...
	result=process_arguments(argc,argv);
		...
		/* Commented out Original Version and Modification Variables as changes
                   to the 'check_nrpe' build can be done independently of the NRPE Agent. - [R. Irujo] - 05.09.2012 */ 
  		/* printf("Version: %s\n",PROGRAM_VERSION); */
	        /* printf("Last Modified: %s\n",MODIFICATION_DATE); */
                printf("Custom NRPE Plugin for the [mon-nrpe-plugin-2.12] RPM build\n");
		printf("Version: v1.1\n");
		printf("Last Modified: 06.11.2012 - [R.Irujo]\n");
	...
	if(use_ssl==TRUE){
		...
		if((ctx=SSL_CTX_new(meth))==NULL){
			...
			/* Exit code changed to STATE_UNKNOWN - [R. Irujo] - 05.09.2012 */
			/* exit(STATE_CRITICAL); */
			exit(STATE_UNKNOWN);
		        }
...
#endif
				/* Result value changed to STATE_UNKNOWN - [R. Irujo] - 05.09.2012 */
